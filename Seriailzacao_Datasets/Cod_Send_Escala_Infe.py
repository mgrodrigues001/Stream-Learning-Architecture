...
""" 
    ["AGR_a_NB", "AGR_a_redu.csv"],
    ["AGR_a_ARF", "AGR_a_redu.csv"],
    ["AGR_a_HT", "AGR_a_redu.csv"],
    ["AGR_a_SRP", "AGR_a_redu.csv"],
    ["AGR_g_NB", "AGR_g_redu.csv"],
    ["AGR_g_ARF", "AGR_g_redu.csv"],
    ["AGR_g_HT", "AGR_g_redu.csv"],
    ["AGR_g_SRP", "AGR_g_redu.csv"],
    ["airlines_NB", "airlines_redu.csv"],
    ["airlines_ARF", "airlines_redu.csv"],
    ["airlines_HT", "airlines_redu.csv"],
    ["airlines_SRP", "airlines_redu.csv"],
    ["youchoose_NB", "youchoose_redu.csv"], 
    ["youchoose_ARF", "youchoose_redu.csv"],
    ["youchoose_HT", "youchoose_redu.csv"],
    ["youchoose_SRP", "youchoose_redu.csv"]
"""
...
for ex in experimentos:
    ...
    descricao = 'Teste de escalabilidade Inferencia'
    n_processes = 6
    shape_df = df.shape
    atualiza_model_up = 1000
    ...
    print(f'Taxa de serializao: {taxa_metrica}')
    ...
    r_taxa = requests.post(url_cont, json=(atualiza_model_up))
    ...
    def get_lag(list_lag):
        try:
            kubectl_command = ["kubectl", "exec", "-it", "pods/kafka-broker-78599d594c-zrp96", "--namespace=kafka", "--", "kafka-consumer-groups.sh", "--bootstrap-server", "localhost:9092", "--describe", "--group", "update"]
    ...
    def inf(t, lista, tempo, metrica, grafico, list_lag):
        ...
        producer = Producer({'bootstrap.servers': kafka_bootstrap_servers, "queue.buffering.max.messages": 10000000,})
    ...
    """ ------------------- Inicio do Processo ------------------- """
    print(f'Executando Experimento {exp}...')
    ...
    path = 'arq_logs/logs_exp_escalab/Exp_escalabilidade.csv'
    isFile = os.path.isfile(path)

    if isFile:
        cont_linhas = subprocess.run(["wc", "-l", "arq_logs/logs_exp_escalab/Exp_escalabilidade.csv"], capture_output=True, text=True)
